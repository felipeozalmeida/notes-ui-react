...
const NoteForm: FC<NoteFormProps> = ({ onCreate, onCancel }) => {
  ...
  return <form className="flex flex-col gap-3 items-stretch justify-center" onSubmit={handleSubmit}>
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch sm:justify-between">
      <InputSelect className="sm:basis-4/12" label="Priority" name="priority" placeholder="select a priority" value={formData.priority} options={Object.values(PRIORITY)} onChange={handleChange} />
      <InputSelect className="sm:basis-8/12" label="Category" name="category" placeholder="select a category" value={formData.category} options={Object.values(CATEGORY)} onChange={handleChange} />
    </div>
